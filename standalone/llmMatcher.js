...
export async function matchQuoteItemsWithLLM(userInput, priceList, options = {}) {
  ...
  if (!apiKey) {
    throw new Error('OpenAI API key required. Go to Settings → Preferences and add your key.');
  }
  if (!userInput?.trim()) {
...
export async function matchQuoteItemsViaBackend(userInput, priceList, backendUrl = 'http://localhost:3001') {
  const response = await fetch(`${backendUrl}/api/ai/generate-quote-items`, {
    method: 'POST',
  ...
  if (!response.ok) {
    ...
    throw new Error(err.error || err.details || `Backend AI request failed (${response.status})`);
  }
  const data = await response.json();
  return data.items || [];
}
