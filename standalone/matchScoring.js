 ...
 * No API key. Tuned for sales shorthand (cub pres, spa boy, onzen, etc.).
 */
...
export function lexicalScore(query, itemName) {
  ...
  for (const qt of qTokens) {
    ...
    weight += tokenWeight;
    const direct = iTokens.some(
      (it) => it === qt || it.includes(qt) || qt.includes(it)
    );
...
export function extractContext(itemName) {
  ...
  const ctx = { series: null, model: null, tier: null, line: null };
...
export function mergeContext(existing, itemName) {
...
export function contextBoost(context, itemName) {
...
export function combineScores(embedding, lexical, phraseBoost, ctxBoost) {
  return 0.3 * embedding + 0.55 * lexical + phraseBoost + ctxBoost;
}

export const MIN_FINAL_SCORE = 0.38;
export const MIN_LEXICAL_OR_EMBEDDING = { lexical: 0.2, embedding: 0.42 };
export const MIN_MARGIN = 0.06;

export function pickBestMatch(scored) {
  ...
  if (best.lexical < MIN_LEXICAL_OR_EMBEDDING.lexical && best.embedding < MIN_LEXICAL_OR_EMBEDDING.embedding) {
    return null;
  }
  if (second && best.final - second.final < MIN_MARGIN && best.lexical < 0.45) {
    return null;
  }
  return best;
}
