...
export function parseQuantity(segment) {
  ...
  const numMatch = segment.match(/(\d+(?:\.\d+)?)\s*(?:x|Ã—)?/);
  if (numMatch) {
...
export function splitSegments(userInput) {
  ...
  // "summit signature grey cover" â†’ "summit signature, grey cover"
...
/** Rule-based quote matching â€” no API keys or embedding models required */
